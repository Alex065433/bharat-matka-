...
async function startServer() {
  ...
  // API: AI Statistical Explanation Endpoint (Grounded in provided statistical facts)
  app.post('/api/ai/explain', async (req, res) => {
    ...
    const buildFallbackExplanation = () => {
      ...
      return `AI Historical Pattern Analysis for ${marketName} (${predictionDate}):\n\n` +
        `The statistical model (${modelVersion}) evaluated ${trainingDataCount} verified historical draws.\n\n` +
    ...
    if (!apiKey) {
      return res.json({ explanation: buildFallbackExplanation(), source: 'statistical_engine_fallback' });
    }
...
Analyze the following MATHEMATICALLY CALCULATED data from the model (${modelVersion}) on market "${marketName}" for target date "${predictionDate}" trained on ${trainingDataCount} prior historical draws.
...
6. End with the mandatory reminder: "These are statistical model outputs based on historical data. They are not guaranteed future results."`;
      ...
      return res.json({ explanation: text, source: 'gemini_ai' });
    } catch (err: any) {
      console.warn('Gemini API call failed, falling back to deterministic explanation:', err?.message);
      return res.json({ explanation: buildFallbackExplanation(), source: 'statistical_engine_fallback' });
    }
  });
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`Bharat Matka AI Analytics server running on http://0.0.0.0:${PORT}`);
  });
}
